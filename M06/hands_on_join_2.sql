...
-- 1. List order id, product id, product name, quantity, order price of all orders
    SELECT o.order_id, oi.product_id, p.name, oi.quantity, oi.unit_price, oi.quantity * oi.unit_price as total
    FROM orders o
    JOIN order_items oi ON o.order_id = oi.order_id
    JOIN products p ON oi.product_id = p.product_id;


-- 2. List order id, order date, customer name, and status of all orders
    SELECT o.order_id, o.order_date, c.first_name, c.last_name, os.name
    FROM orders o
    JOIN customers c ON o.customer_id = c.customer_id
    JOIN order_statuses os ON o.status = os.order_status_id;

-- 3. Connect order item and order item notes table, and display all columns.
    SELECT oi.* , oin.*
    FROM order_items oi
    JOIN order_item_notes oin ON oi.order_id = oin.order_id;


-- 4. List customer id, customer name, order id of all orders, and sort by customer id
    SELECT c.customer_id, c.first_name, c.last_name, o.order_id
    FROM customers c
    JOIN orders o ON c.customer_id = o.customer_id
    ORDER BY c.customer_id;

-- 5. List customer id, customer name, order id of all customers, and sort by customer id
    SELECT c.customer_id, c.first_name, c.last_name, o.order_id
    FROM customers c
    LEFT JOIN orders o ON c.customer_id = o.customer_id
    ORDER BY c.customer_id;

-- 6. List customer id, customer name, order id, shipper name of all orders, and sort by customer id
    SELECT c.customer_id, c.first_name, c.last_name, o.order_id, s.name
    FROM customers c
    JOIN orders o ON c.customer_id = o.customer_id
